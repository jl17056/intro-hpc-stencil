             ...
             double* image, double* tmp_image)
{
  for (int i = 1; i < ny + 1; ++i) {
    for (int j = 1; j < nx + 1; ++j) {
      tmp_image[j + i * height] =  (image[j + i * height] * 3.0 / 5.0) + (image[j + (i - 1) * height] * 0.5 / 5.0) + (image[j + (i + 1) * height] * 0.5 / 5.0) + (image[j - 1 + i * height] * 0.5 / 5.0) + (image[j + 1 + i * height] * 0.5 / 5.0);
    }
  }
}
